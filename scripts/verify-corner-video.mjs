// Isolated browser playback checks; no hosted data or payments.
...
try {
  ...
  process.env.PAPER_DEMO_SLOT = 'tsq-007';
  await import('./capture-video-demos.mjs');
  console.log('TSQ-007 panel and 3D video playback passed on desktop and mobile.');
} catch (error) {
  await mkdir('artifacts/media', { recursive: true });
  await writeFile('artifacts/media/corner-video-server.log', logs);
  throw error;
} finally {
