...
3. Enable email OTP authentication. Configure the email template to display `{{ .Token }}`. This app verifies the code server-side and issues its own opaque HttpOnly/SameSite session; it does not use magic-link callbacks or OAuth. Set the Supabase Site URL to the canonical public HTTPS `APP_ORIGIN`; allow only that origin in auth redirects. Add `http://localhost:3000` only to a separate development project.
4. Sign in once through real OTP with the intended operator email. In the trusted SQL console, promote that existing account with a parameterized equivalent of `UPDATE accounts SET role='admin' WHERE email='your-verified-address';`. Record this bootstrap action in your operator change log. There is no production auto-admin email. Never promote an unverified arbitrary signup.
...
- Sign in through real email OTP, upload an image, prove private access from another session fails, and continue to checkout without an operator approval step.
...
Keep live charging disabled until the operator has explicitly obtained merchant acceptance for this exact explorable virtual-advertising product, finalized operator identity/support/privacy/retention/refund terms, and completed the above tests. Then provision separate live keys/product/webhook **and a separate clean database/storage project**, run migrations, and set `PAYMENT_MODE=dodo-live` and `DODO_LIVE_ACCEPTANCE=confirmed-for-this-product`. Production startup rejects a database containing orders from another payment mode. Never turn test allocations into live sponsor history; retain the test deployment separately for reconciliation. That environment flag is an operator attestation, not proof of approval. This build has not enabled it or published the site.
