...
The application already has a Vercel deployment. This video-premium release requires its database update before a new production push; see [release verification](video-release-verification.md) for current status. Audit screenshots and generated test/provider artifacts remain local under ignored artifacts. Credentials remain in ignored environment files or the host's secret manager.
