...
- Images use a 4 MB proxy; videos up to 50 MB use signed resumable uploads directly to private Supabase storage. Run [the video database/storage update](../scripts/production-video-upgrade.sql) before deploying the pricing code, then verify hosted uploads and playback. See [media verification and limits](billboard-media.md). [Function limits](https://vercel.com/docs/functions/limitations).
...
The video-premium migration was applied and the release deployed on 13 September 2026; see [release verification](video-release-verification.md). vercel.json runs an opt-in release check before the normal build. The migration is skipped unless PAPER_APPLY_VIDEO_UPGRADE=1 is explicitly supplied to a production build; it uses existing host secrets in place. Preview builds cannot run it. Audit screenshots and generated test/provider artifacts remain local under ignored artifacts. Credentials remain in ignored environment files or the host's secret manager.
