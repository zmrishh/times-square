...
- Review Vercel's request-body limit against the current 5 MiB upload allowance. Files near that limit cannot use the existing proxied upload route unchanged if they exceed the platform limit. A production Vercel upload path needs a tested lower allowance or a private direct-upload/validation flow; do not make the bucket public as a workaround. [Function limits](https://vercel.com/docs/functions/limitations).
